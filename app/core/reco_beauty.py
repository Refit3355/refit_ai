...
def load_frames_beauty() -> dict:
    ...
    # 뷰티 카테고리만 (0~5)
    df_product = df_product[df_product["CATEGORY_ID"].between(0, 5)].reset_index(drop=True)
...
_RAW_CONCERN_TO_EFFECTS: Dict[str, List[str]] = {
    "여드름":["진정","여드름 완화"],
    "미백":["미백"],
    "피지":["진정"],
    "속건조":["보습"],
...
def build_query_text(member_id: int, frames: dict, prefer_category_name: str = None) -> dict:
    ...
    skin_list   = pick_labels(skin_row, SKIN_FLAG_2_LABEL)
...
def build_faiss_index(dp: pd.DataFrame):
    ...
    index = faiss.IndexIDMap(faiss.IndexFlatIP(vecs.shape[1]))
...
# 메인 추천
# -----------------------------
def recommend_beauty(member_id: int,
                     frames: dict,
    ...
    dp = dp[dp["CATEGORY_ID"].between(0, 5)].reset_index(drop=True)
