...
# -----------------------------
ALPHA  = 0.5   # 임베딩 유사도
BETA   = 0.7   # 고민→효능 매칭 (헬스 전용)
DELTA  = 0.2   # 인기도(log1p(UNITS_SOLD))
LAMBDA = 0.5   # 날씨 보정 (헬스 룰)
TOPK   = 200
FINAL  = 10

# -----------------------------
# 데이터 적재(건강식품 범위: CATEGORY_ID 8~11)
# -----------------------------
def load_frames_health() -> dict:
    ...
    if "CATEGORY_ID" in df_product.columns:
        df_product = df_product[df_product["CATEGORY_ID"].between(8, 11)].reset_index(drop=True)
...
def get_health_effect_vocab(df_effect: pd.DataFrame) -> set:
    if df_effect.empty or not {"EFFECT_NAME","BH_TYPE"}.issubset(df_effect.columns):
        return set()
...
_RAW_HEALTH_CONCERN_TO_EFFECTS: Dict[str, List[str]] = {
    "눈 건강": ["눈 건강"],
    "활력":   ["활력"],
    "면역력 증진": ["면역력 증진"],
    "장 건강": ["장 건강"],
...
_RAW_WEATHER_RULES = [
    {"cond": lambda w: _nz(w.get("pm25"), 0.0) >= 35.0,
     "effects": ["항산화"], "bonus": 0.7},
    {"cond": lambda w: _nz(w.get("uvi"), 0.0) >= 7.0,
     "effects": ["항산화"], "bonus": 0.5},
...
def normalize_rule_effects(rules: List[dict], vocab: set) -> List[dict]:
    out=[]
    for r in rules:
        effs = normalize_health_effect_names(r["effects"], vocab)
...
def build_prod_effects(frames: dict) -> Dict[int, List[str]]:
    ...
    if eff.empty or pe.empty:
        return {}
    need_cols_eff = {"EFFECT_ID","EFFECT_NAME","BH_TYPE"}
    if not need_cols_eff.issubset(eff.columns):
        return {}
    eff_h = eff[eff["BH_TYPE"]==1][["EFFECT_ID","EFFECT_NAME"]].copy()
    merged = (
        pe.merge(eff_h, on="EFFECT_ID", how="inner")
...
def recommend_health(member_id: int,
    ...
    if "CATEGORY_ID" in dp.columns:
        dp = dp[dp["CATEGORY_ID"].between(8, 11)].reset_index(drop=True)
